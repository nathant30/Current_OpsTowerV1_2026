...
import type { PayMayaPaymentRequest, PaymentCallback } from '@/types/payment';

interface PayMayaIntegrationProps {
  amount: number;
  description: string;
...
export const PayMayaIntegration: React.FC<PayMayaIntegrationProps> = ({
  amount,
  description,
  referenceNumber,
  onSuccess,
  ...
  useEffect(() => {
    if (paymentState === 'processing') {
      const handleMessage = (event: MessageEvent) => {
        if (event.data.type === 'paymaya-payment-callback') {
  ...
  const handlePayment = async () => {
    ...
    try {
      const paymentRequest: PayMayaPaymentRequest = {
        amount,
        description,
        referenceNumber,
      ...
      const response = await fetch('/api/payments/paymaya/initiate', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to initiate PayMaya payment');
      }
    ...
    <Card variant="outlined" className="w-full">
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle>PayMaya Payment</CardTitle>
            <p className="text-sm text-neutral-600 mt-1">
              Pay securely with your PayMaya account
            </p>
          </div>
        </div>
      </CardHeader>
            ...
            <div className="space-y-3">
              <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                <p className="text-sm text-green-800">
                  You will be redirected to PayMaya to complete your payment securely.
                </p>
              </div>
              <Button
                fullWidth
            ...
            <div className="space-y-3">
              <div className="flex flex-col items-center justify-center py-6">
                ...
                <p className="text-sm text-neutral-600 text-center">
                  Complete the payment in the PayMaya window.
                  <br />
