...
interface EnhancedUser {
  ...
  firstName: string;
  lastName: string;
  roles: Array<{ role: { name: string }; isActive: boolean }>;
  permissions: string[];
...
export const POST = asyncHandler(async (request: NextRequest) => {
  ...
  try {
    ...
    const tokens = await authManager.generateTokens({
      userId: user.id,
      userType: 'operator',
      role: user.role as 'regional_manager' | 'dispatcher' | 'safety_monitor' | 'analyst' | 'admin' | 'api_client',
    ...
    // Prepare response user object (matching EnhancedUser structure)
    const responseUser = {
      id: user.id,
      email: user.email,
      firstName: user.firstName,
      lastName: user.lastName,
      roles: [{ role: { name: user.role }, isActive: true }],
      permissions: user.permissions || [],
      allowedRegions: user.regionId ? [user.regionId] : [],
      piiScope: 'masked' as const,
    ...
    // Return AuthenticationResponse structure
